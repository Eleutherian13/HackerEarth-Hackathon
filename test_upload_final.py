...
def login_user(email, password):
    ...
    if response.status_code == 200:
        ...
        print(f"[FAIL] Login failed: {response.text[:300]}")
        return None
...
def upload_pdf(token, pdf_path):
    ...
    if response.status_code == 200:
        ...
        print(f"[OK] Upload successful: {doc_id}")
        return doc_id
    else:
        print(f"[FAIL] Upload failed: {response.text[:300]}")
        return None
    ...
    while time.time() - start < max_wait:
        ...
        if response.status_code == 200:
            ...
            if status == "PENDING_REVIEW":
                print(f"[OK] Processing complete! Document ready for review")
                return True
            elif status == "FAILED":
                print(f"[FAIL] Processing failed: {data.get('error_message')}")
                return False
...
def main():
    ...
    try:
        ...
        print("\n" + "=" * 70)
        if success:
            print("[OK] SUCCESS: Document uploaded and LLM extraction completed!")
