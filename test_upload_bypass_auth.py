"""
End-to-end test using AUTH_BYPASS - no login needed.
This test directly uploads and analyzes documents without authentication.
"""

import sys
import time
import json
import requests
from pathlib import Path
from reportlab.pdfgen import canvas
from reportlab.lib.pagesizes import letter

API_URL = "http://localhost:8000/api/v1"
TEST_PDF = Path("test_judgment.pdf")

def create_test_pdf():
    """Create a valid test PDF with court judgment content."""
    print("[1/6] Creating test PDF...")
    c = canvas.Canvas(str(TEST_PDF), pagesize=letter)
    c.setFont("Helvetica", 12)
    
    c.drawString(50, 750, "HIGH COURT OF DELHI")
    c.drawString(50, 700, "CIVIL APPEAL NO. 2024-001")
    c.drawString(50, 650, "Dated: 2024-05-01")
    c.drawString(50, 550, "PETITIONER: State of Delhi")
    c.drawString(50, 500, "RESPONDENT: ABC Corporation")
    c.drawString(50, 400, "JUDGMENT")
    c.drawString(50, 350, "This court hereby directs the respondent to comply with all")
    c.drawString(50, 330, "statutory obligations within 30 days of the order.")
    c.drawString(50, 310, "The respondent shall submit a compliance report within 45 days.")
    c.drawString(50, 290, "Failure to comply will result in contempt proceedings.")
    c.save()
    print("✓ Test PDF created")
    return TEST_PDF

def upload_document(pdf_path: Path):
    """Upload PDF document (no auth required - uses AUTH_BYPASS)."""
    print(f"\n[2/6] Uploading PDF: {pdf_path.name}")
    
    with open(pdf_path, 'rb') as f:
        files = {'file': f}
        # No authorization header - AUTH_BYPASS will handle it
        response = requests.post(
            f"{API_URL}/documents/upload",
            files=files,
            timeout=30
        )
    
    print(f"Status: {response.status_code}")
    if response.status_code != 200:
        print(f"Response: {response.text[:500]}")
        return None
    
    data = response.json()
    document_id = data.get("document_id")
    print(f"✓ Upload successful, document_id: {document_id}")
    print(f"  Filename: {data.get('filename')}")
    print(f"  Status: {data.get('status')}")
    return document_id

def check_document_status(document_id: str, max_wait: int = 90):
    """Check document processing status and wait for LLM extraction."""
    print(f"\n[3/6] Checking document status (max {max_wait}s wait)...")
    
    start_time = time.time()
    wait_count = 0
    
    while time.time() - start_time < max_wait:
        response = requests.get(
            f"{API_URL}/documents/{document_id}",
            timeout=10
        )
        
        if response.status_code == 200:
            data = response.json()
            status = data.get("status")
            print(f"  [{wait_count}] Status: {status}")
            
            if status == "PENDING_REVIEW":
                print("✓ Document reached PENDING_REVIEW (LLM extraction complete!)")
                return True, data
            elif status == "FAILED":
                error = data.get("error_message")
                print(f"✗ Processing failed: {error}")
                return False, data
        else:
            print(f"  API error: {response.status_code}")
        
        time.sleep(3)
        wait_count += 1
    
    print(f"✗ Timeout waiting for processing (checked {wait_count} times)")
    return False, None

def get_extracted_fields(document_id: str):
    """Retrieve extracted fields from LLM."""
    print(f"\n[4/6] Retrieving extracted fields...")
    
    response = requests.get(
        f"{API_URL}/documents/{document_id}/extractions",
        timeout=10
    )
    
    print(f"Status: {response.status_code}")
    
    if response.status_code == 200:
        data = response.json()
        fields_count = data.get("total_fields", 0)
        print(f"✓ Retrieved {fields_count} extracted fields")
        
        fields = data.get("fields", [])
        for i, field in enumerate(fields[:5]):
            value_preview = str(field.get('value', ''))[:80]
            print(f"  [{i+1}] {field.get('field_type')}: {value_preview}")
        
        return True, data
    else:
        print(f"⚠ Status {response.status_code}: {response.text[:300]}")
        return False, None

def get_document_pages(document_id: str):
    """Get document pages and check if they exist."""
    print(f"\n[5/6] Checking document pages...")
    
    response = requests.get(
        f"{API_URL}/documents/{document_id}/pages",
        timeout=10
    )
    
    print(f"Status: {response.status_code}")
    
    if response.status_code == 200:
        data = response.json()
        page_count = data.get("total_pages", 0)
        print(f"✓ Document has {page_count} pages")
        
        pages = data.get("pages", [])
        for i, page in enumerate(pages[:3]):
            text_len = len(page.get('raw_text', ''))
            print(f"  [Page {i+1}] {text_len} chars of text")
        
        return True, data
    else:
        print(f"⚠ Status {response.status_code}: {response.text[:300]}")
        return False, None

def main():
    """Run the end-to-end test with AUTH_BYPASS."""
    print("=" * 70)
    print("LAOS Document Upload + LLM Extraction E2E Test (AUTH_BYPASS)")
    print("=" * 70)
    
    try:
        # Step 1: Create test PDF
        pdf_path = create_test_pdf()
        
        # Step 2: Upload document (no auth needed)
        document_id = upload_document(pdf_path)
        if not document_id:
            print("✗ Upload failed")
            return False
        
        # Step 2.5: Check document pages were created
        print(f"\n[2.5/6] Checking extracted pages from PDF...")
        pages_ok, pages_data = get_document_pages(document_id)
        if pages_ok:
            print("  ✓ Document pages extracted successfully")
        
        # Step 3: Check status and wait for processing
        print(f"\n[3/6] Waiting for LLM analysis...")
        success, status_data = check_document_status(document_id, max_wait=90)
        
        if not success:
            print("⚠ Document processing did not reach PENDING_REVIEW")
            if status_data:
                print(f"  Final status: {status_data.get('status')}")
                if status_data.get('error_message'):
                    print(f"  Error: {status_data.get('error_message')}")
                print(f"  Full response: {json.dumps(status_data, indent=2)[:500]}")
        
        # Step 4: Try to get extracted fields (may not exist if processing failed)
        if success:
            get_extracted_fields(document_id)
        
        # Cleanup
        if TEST_PDF.exists():
            TEST_PDF.unlink()
        
        print("\n" + "=" * 70)
        if success:
            print("✓ END-TO-END TEST PASSED!")
            print("Document upload + LLM extraction pipeline is working!")
        else:
            print("⚠ TEST COMPLETED WITH WARNINGS")
            print("Document was uploaded but LLM analysis may have failed.")
            print("Check backend logs for detailed error information.")
        print("=" * 70)
        return success
        
    except Exception as e:
        print(f"\n✗ Test failed with exception: {e}")
        import traceback
        traceback.print_exc()
        return False

if __name__ == "__main__":
    success = main()
    sys.exit(0 if success else 1)
